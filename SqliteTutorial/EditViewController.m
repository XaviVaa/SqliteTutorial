...
- (void)viewDidLoad
{
    ...
    nombreTextField.text = auxTutorial.nombre;
    
    if([auxTutorial.sistema isEqualToString:@"iOS"])
    {
        sistemaSegControl.selectedSegmentIndex = 0;
    } else {
        sistemaSegControl.selectedSegmentIndex = 1;
    }
    
    if(auxTutorial.terminado)
    {
        terminadoSwitch.on = YES;
    } else {
        terminadoSwitch.on = NO;
    }
...
- (IBAction)updateTutorial
{
    ...
    if(![nombreTextField.text isEqualToString:@""])
    {
        ...
        if(sqlite3_open([appDelegate.databasePath UTF8String], &database) == SQLITE_OK) {
            ...
            NSString *sqlStatement = [NSString stringWithFormat:@"UPDATE Tutoriales SET \"sistema\" = \"%@\",\"nombre\" = \"%@\", \"terminado\" = \"%d\" WHERE id_tutorial = \"%d\"",
                                      auxTutorial.sistema,
                                      auxTutorial.nombre,
